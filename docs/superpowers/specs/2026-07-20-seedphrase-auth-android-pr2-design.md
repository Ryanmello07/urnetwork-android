...
- Per-method content:
  ...
  - **Wallet**: Phantom/Solflare connect buttons → `Api.authWalletChallenge(...)` → deep-link back with signature → `Api.addAuth(AddAuthArgs(walletAuth = ...))`. Reuses the existing wallet-connect plumbing already proven in `SolanaWalletAuth.kt`/`LoginCreateNetworkViewModel.kt` (login-time wallet connect) — this is the same flow, just calling `addAuth` instead of `networkCreate`/`authLogin` at the end.
...
- One "Add Sign-In Method" button, shown only for Email/Seedphrase (Apple/Google/Wallet complete on their own native callback, matching iOS).
- On sheet dismiss: cancel any in-flight wallet-challenge-poll coroutine and clear the pending-signature handler — **this must be complete on Android**, unlike iOS's own shipped version, which the earlier iOS code-review pass found left one code path (the "Sign with Wallet" button's own task) still uncancelled even after iOS's fix. Android's plan must cancel both the initial connect-poll and the post-connect sign-poll on dismiss.
